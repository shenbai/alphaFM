...
public:
	ftrl_trainer(const ftrl_trainer_option& opt);
...
ftrl_trainer::ftrl_trainer(const ftrl_trainer_option& opt) {
	w_alpha = opt.w_alpha;
	w_beta = opt.w_beta;
	w_l1 = opt.w_l1;
	w_l2 = opt.w_l2;
	...
	pModel = new ftrl_model(opt.factor_num, opt.space_size, opt.init_mean,
			opt.init_stdev);
}
	...
	if (line_num % 1000 < 5) {	// val
		...
		for (int i = 0; i <= xLen; ++i) {
			...
			if ((i < xLen && k1) || (i == xLen && k0)) {
				mu.mtx.lock();
				double w_gi = mult * xi;
				double w_si = 1 / w_alpha
						* (sqrt(mu.w_ni + w_gi * w_gi) - sqrt(mu.w_ni));
		...
		for (int i = 0; i < xLen; ++i) {
			...
			for (int f = 0; f < pModel->factor_num; ++f) {
				...
				double v_sif = 1 / v_alpha
						* (sqrt(v_nif + v_gif * v_gif) - sqrt(v_nif));
